...
#[inline(always)]
fn sext(operand: u16, w: u16, b: u8) -> u16 {
    let m = 1 << (b - 1);
    let x = w & ((1 << b) - 1);
    operand.wrapping_add_signed((x ^ m) as i16 - (m) as i16)
}

fn to_char(v: u16) -> char {
    (v & 0x7f) as u8 as char
}
...
impl LC3 {
    ...
    pub fn setreg(&mut self, r: usize, val: u16) {
        self.reg[r] = val;
    }
    pub fn getreg(&self, r: usize) -> u16 {
        self.reg[r]
    }
    pub fn execute(&mut self) {
        ...
        while !self.halted {
            ...
            match w >> 12 {
                ...
                ADD => self.cset(a, self.reg[b].wrapping_add(self.add_and_arg(w))),
                AND => self.cset(a, self.reg[b] & self.add_and_arg(w)),
                LD => self.cset(a, self.rmem(sext(self.pc, w, 9))),
                LDR => self.cset(a, self.rmem(sext(self.reg[b], w, 6))),
                LDI => self.cset(a, self.rmem(self.rmem(sext(self.pc, w, 9)))),
                LEA => self.cset(a, sext(self.pc, w, 9)),
                ST => self.wmem(a, sext(self.pc, w, 9)),
                STR => self.wmem(a, sext(self.reg[b], w, 6)),
                STI => self.wmem(a, self.rmem(sext(self.pc, w, 9))),
                BR => {
                    if w & self.regcc != 0 {
                        self.pc.wrapping_add(sext(self.pc, w, 9));
                    }
                }
    ...
    fn jsr(&mut self, w: u16, r: usize) {
        ...
        if w & 0x800 == 0 {
            ...
            self.pc.wrapping_add(sext(self.pc, w, 11));
        }
    }
