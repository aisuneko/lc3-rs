...
fn main() -> Result<()> {
    ...
    for (i, &w) in program.iter().enumerate() {
        ...
        match w >> 12 {
            ...
            LD => println!("ld  ${} {:#06x}", r, sext(pc, w, 9)),
            LDR => println!("ldr ${} ${} {:#06x}", r, r2, sext(0, w, 6)),
            LDI => println!("ldi ${} {:#06x}", r, sext(pc, w, 9)),
            LEA => println!("lea ${} {:#06x}", r, sext(pc, w, 9)),
            ST => println!("st  ${} {:#06x}", r, sext(pc, w, 9)),
            STR => println!("str ${} ${} {}", r, r2, sext(0, w, 6)),
            STI => println!("sti ${} {:#06x}", r, sext(pc, w, 9)),
            JMP => println!("jmp ${}", r2),
            BR => {
                let n = if w & 0x800 != 0 { "n" } else { "" };
                let z = if w & 0x400 != 0 { "z" } else { "" };
                let p = if w & 0x200 != 0 { "p" } else { "" };
                println!("br{}{}{} {:#06x}", n, z, p, sext(pc, w, 9));
            }
            JSR => {
                if w & 0x800 == 0 {
                    println!("jsr ${}", r2);
                } else {
                    println!("jsr {:#06x}", sext(pc, w, 11));
                }
            }
            TRP => match w & 0xff {
                GETC => println!("getc"),
